...
const NOTES: Note[] = [{
  ...
  content: <div className="space-y-4" data-magicpath-id="0" data-magicpath-path="PersonalWebsite.tsx">
        ...
        <p style={{
      display: "none"
    }} data-magicpath-id="5" data-magicpath-path="PersonalWebsite.tsx">
...
export const PersonalWebsite = () => {
  ...
  const renderContent = () => {
    if (activeNote.id === 'about') {
      return <>
          {activeNote.content}
          <div className="mt-8 pt-8 border-t border-gray-200 space-y-2" data-magicpath-id="23" data-magicpath-path="PersonalWebsite.tsx">
            {NOTES.filter(n => n.id !== 'about').map(note => <div key={note.id} data-magicpath-id="24" data-magicpath-path="PersonalWebsite.tsx">
                <button onClick={() => setActiveNoteId(note.id)} className="text-gray-800 underline hover:text-gray-600 transition-colors text-left" data-magicpath-id="25" data-magicpath-path="PersonalWebsite.tsx">
                  {note.title}
                </button>
              </div>)}
          </div>
  ...
  // @return
  return <div className="flex h-screen w-full bg-white overflow-hidden font-sans text-gray-900" data-magicpath-id="26" data-magicpath-path="PersonalWebsite.tsx">
      {/* Sidebar */}
      <div className="w-48 border-r border-gray-200 flex flex-col" data-magicpath-id="27" data-magicpath-path="PersonalWebsite.tsx">
        <div className="p-4 border-b border-gray-200" data-magicpath-id="28" data-magicpath-path="PersonalWebsite.tsx">
          <h1 className="text-lg font-semibold" data-magicpath-id="29" data-magicpath-path="PersonalWebsite.tsx">Jessica Mustali</h1>
        </div>

        <div className="flex-1 overflow-y-auto" data-magicpath-id="30" data-magicpath-path="PersonalWebsite.tsx">
          {NOTES.map(note => <button key={note.id} onClick={() => setActiveNoteId(note.id)} className={`w-full text-left px-4 py-3 text-sm border-b border-gray-100 hover:bg-gray-50 transition-colors ${activeNoteId === note.id ? 'bg-gray-50' : ''}`} data-magicpath-id="31" data-magicpath-path="PersonalWebsite.tsx">
              {note.title}
            </button>)}
        </div>
      </div>
      ...
      {/* Main Content */}
      <div className="flex-1 flex flex-col bg-white h-full overflow-hidden" data-magicpath-id="32" data-magicpath-path="PersonalWebsite.tsx">
        <AnimatePresence mode="wait" data-magicpath-id="33" data-magicpath-path="PersonalWebsite.tsx">
